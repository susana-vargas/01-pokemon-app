...
describe('CreatePokemonDto', () => {
  ...
  it('should be invalid with non-string sprites', async () => {
    ...
    expect(spritesError).toBeDefined(); //verifica que no sea undefined
  });
  ...
  it('should be valid with string sprites', async () => {
    ...
    const spritesError = errors.find((error) => error.property === 'sprites');
    expect(spritesError).toBe(undefined); //No tiene valor (es undefined)
    expect(spritesError).toBeUndefined(); //que sea undefined exactamente
  });
});
